...
#include "cpu/host_gamma_correction.hpp"
...
int main(int argc, char** argv)
{
    ...
    if (input == 1)
    {
        ...
        {   
            if (channel == 1)
            {
                ...
                rppi_gamma_correction_u8_pln1_host(srcPtr, srcSize, dstPtr, gamma);
            ...
            else if (channel == 3)
            {
                ...
                start = high_resolution_clock::now();
                rppi_gamma_correction_u8_pln3_host(srcPtrTemp, srcSize, dstPtrTemp, gamma);
                stop = high_resolution_clock::now();
        ...
        {   
            ...
            else if (channel ==3)
            {
                printf("\nExecuting pkd3...\n");
                start = high_resolution_clock::now();
                rppi_gamma_correction_u8_pkd3_host(srcPtr, srcSize, dstPtr, gamma);
                stop = high_resolution_clock::now();
    ...
    if (matrix == 1)
    {
        ...
        rppi_gamma_correction_u8_pln1_host(srcPtr, srcSize, dstPtr, gamma);
        printf("\n\nOutput of gamma correction Modification:\n");
    ...
    else if (matrix == 2)
    {
        ...
        if (type == 1)
        {
            ...
            printf("\n\nInput:\n");
            displayPlanar(srcPtr, srcSize, channel);
            rppi_gamma_correction_u8_pln3_host(srcPtr, srcSize, dstPtr, gamma);
            printf("\n\nOutput of gamma correction Modification:\n");
            displayPlanar(dstPtr, srcSize, channel);
        }
        else if (type == 2)
