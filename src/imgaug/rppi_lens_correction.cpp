...
#include "cpu/host_lens_correction.hpp"
...
int main(int argc, char** argv)
{
    ...
    if (input == 1)
    {
        ...
        {   
            if (channel == 1)
            {
                ...
                rppi_lens_correction_u8_pln1_host(srcPtr, srcSize, dstPtr, strength, zoom);
            ...
            else if (channel == 3)
            {
                ...
                start = high_resolution_clock::now();
                rppi_lens_correction_u8_pln3_host(srcPtrTemp, srcSize, dstPtrTemp, strength, zoom);
                stop = high_resolution_clock::now();
        ...
        {   
            ...
            else if (channel ==3)
            {
                printf("\nExecuting pkd3...\n");
                start = high_resolution_clock::now();
                rppi_lens_correction_u8_pkd3_host(srcPtr, srcSize, dstPtr, strength, zoom);
                stop = high_resolution_clock::now();
    ...
    else if (matrix == 2)
    {
        ...
        if (type == 1)
        {
            ...
            printf("\n\nInput:\n");
            displayPlanar(srcPtr, srcSize, channel);
            rppi_lens_correction_u8_pln3_host(srcPtr, srcSize, dstPtr, strength, zoom);
            printf("\n\nOutput of lens_correction:\n");
            displayPlanar(dstPtr, srcSize, channel);
        }
        else if (type == 2)
