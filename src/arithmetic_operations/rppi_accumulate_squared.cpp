...
#include "cpu/host_accumulate_squared.hpp"
...
int main(int argc, char** argv)
{
    ...
    if (input == 1)
    {
        ...
        {   
            if (channel == 1)
            {
                ...
                rppi_accumulate_squared_u8_pln1_host(srcPtr, srcSize);
                stop = high_resolution_clock::now();

                imageOut = Mat(srcSize.height, srcSize.width, CV_8UC1, srcPtr);
            ...
            else if (channel == 3)
            {
                ...
                start = high_resolution_clock::now();
                rppi_accumulate_squared_u8_pln3_host(srcPtrTemp, srcSize);
                stop = high_resolution_clock::now();

                rppi_planar_to_packed_u8_pln3_host(srcPtrTemp, srcSize, srcPtr);

                imageOut = Mat(srcSize.height, srcSize.width, CV_8UC3, srcPtr);
            }
        }
        else if (type == 2)
        {   
            ...
            else if (channel ==3)
            {
                printf("\nExecuting pkd3...\n");
                start = high_resolution_clock::now();
                rppi_accumulate_squared_u8_pkd3_host(srcPtr, srcSize);
                stop = high_resolution_clock::now();
    ...
    else if (matrix == 2)
    {
        ...
        if (type == 1)
        {
            ...
            printf("\n\nInput:\n");
            displayPlanar(srcPtr, srcSize, channel);
            rppi_accumulate_squared_u8_pln3_host(srcPtr, srcSize);
            printf("\n\nOutput of Accumulate_squared:\n");
            displayPlanar(srcPtr, srcSize, channel);
        }
        else if (type == 2)
