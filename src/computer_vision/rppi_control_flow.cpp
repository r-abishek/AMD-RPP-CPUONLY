...
#include "cpu/host_control_flow.hpp"
...
{
    control_flow_host<Rpp8u>(static_cast<Rpp8u*>(srcPtr1), static_cast<Rpp8u*>(srcPtr2), srcSize, static_cast<Rpp8u*>(dstPtr),
                             type, 
                             RPPI_CHN_PLANAR, 1);
...
int main(int argc, char** argv)
{
    ...
    int flowType;
    printf("\nEnter flowType: 1 = bitwise_AND, 2 = inclusive_OR, 3 = exclusive_OR, 4 = add, 5 = subtract, 6 = multiply, 7 = min, 8 = max: ");
    scanf("%d", &flowType);
    ...
    if (input == 1)
    {
        ...
        {   
            if (channel == 1)
            {
                ...
                rppi_control_flow_u8_pln1_host(srcPtr1, srcPtr2, srcSize, dstPtr, flowType);
            ...
            else if (channel == 3)
            {
                ...
                start = high_resolution_clock::now();
                rppi_control_flow_u8_pln3_host(srcPtr1Temp, srcPtr2Temp, srcSize, dstPtrTemp, flowType);
                stop = high_resolution_clock::now();
        ...
        {   
            ...
            else if (channel ==3)
            {
                printf("\nExecuting pkd3...\n");
                start = high_resolution_clock::now();
                rppi_control_flow_u8_pkd3_host(srcPtr1, srcPtr2, srcSize, dstPtr, flowType);
                stop = high_resolution_clock::now();
    ...
    else if (matrix == 2)
    {
        ...
        if (type == 1)
        {
            ...
            printf("\n\nInput 2:\n");
            displayPlanar(srcPtr2, srcSize, channel);
            rppi_control_flow_u8_pln3_host(srcPtr1, srcPtr2, srcSize, dstPtr, flowType);
            printf("\n\nOutput of Absolute Difference:\n");
            displayPlanar(dstPtr, srcSize, channel);
        }
        else if (type == 2)
