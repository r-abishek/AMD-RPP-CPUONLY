...
#include "cpu/host_gaussian_image_pyramid.hpp"
...
int main(int argc, char** argv)
{
    ...
    if (input == 1)
    {
        ...
        {   
            if (channel == 1)
            {
                ...
                rppi_gaussian_image_pyramid_u8_pln1_host(srcPtr, srcSize, dstPtr, stdDev, kernelSize);
            ...
            else if (channel == 3)
            {
                ...
                start = high_resolution_clock::now();
                rppi_gaussian_image_pyramid_u8_pln3_host(srcPtrTemp, srcSize, dstPtrTemp, stdDev, kernelSize);
                stop = high_resolution_clock::now();
        ...
        {   
            ...
            else if (channel ==3)
            {
                printf("\nExecuting pkd3...\n");
                start = high_resolution_clock::now();
                rppi_gaussian_image_pyramid_u8_pkd3_host(srcPtr, srcSize, dstPtr, stdDev, kernelSize);
                stop = high_resolution_clock::now();
    ...
    if (matrix == 1)
    {
        ...
        rppi_gaussian_image_pyramid_u8_pln1_host(srcPtr, srcSize, dstPtr, stdDev, kernelSize);
        printf("\n\nOutput of gaussian_image_pyramid:\n");
    ...
    else if (matrix == 2)
    {
        ...
        if (type == 1)
        {
            ...
            printf("\n\nInput:\n");
            displayPlanar(srcPtr, srcSize, channel);
            rppi_gaussian_image_pyramid_u8_pln3_host(srcPtr, srcSize, dstPtr, stdDev, kernelSize);
            printf("\n\nOutput of gaussian_image_pyramid:\n");
            displayPlanar(dstPtr, dstSize, channel);
        }
        else if (type == 2)
