...
#include "cpu/host_fast_corner_detector.hpp"
...
{
    fast_corner_detector_host<Rpp8u>(static_cast<Rpp8u*>(srcPtr), srcSize, static_cast<Rpp8u*>(dstPtr), 
                                       numOfPixels, threshold, 
                                       nonmaxKernelSize, 
                                       RPPI_CHN_PLANAR, 1);
    return RPP_SUCCESS;
}
...
int main(int argc, char** argv)
{
    ...
    unsigned int numOfPixels = 14;
    Rpp8u threshold = (Rpp8u) 5;
    unsigned int nonmaxKernelSize = 15;
    
    printf("\nEnter minimum number of contiguous pixels (12 <= numOfPixels <= 16): ");
    scanf("%d", &numOfPixels);
    Rpp32u thresholdInt;
    printf("\nEnter threshold (0 <= threshold <= 255): ");
    scanf("%d", &thresholdInt);
    threshold = (Rpp8u) thresholdInt;
    printf("\nEnter nonnaxKernelSize: ");
    scanf("%d", &nonmaxKernelSize);
    ...
    if (input == 1)
    {
        ...
        {   
            if (channel == 1)
            {
                ...
                rppi_fast_corner_detector_u8_pln1_host(srcPtr, srcSize, dstPtr, numOfPixels, threshold, nonmaxKernelSize);
            ...
            else if (channel == 3)
            {
                ...
                start = high_resolution_clock::now();
                rppi_fast_corner_detector_u8_pln3_host(srcPtrTemp, srcSize, dstPtrTemp, numOfPixels, threshold, nonmaxKernelSize);
                stop = high_resolution_clock::now();
        ...
        {   
            ...
            else if (channel ==3)
            {
                printf("\nExecuting pkd3...\n");
                start = high_resolution_clock::now();
                rppi_fast_corner_detector_u8_pkd3_host(srcPtr, srcSize, dstPtr, numOfPixels, threshold, nonmaxKernelSize);
                stop = high_resolution_clock::now();
    ...
    //    rppi_fast_corner_detector_u8_pln1_host(srcPtr, srcSize, dstPtr, gaussianKernelSize, stdDev, kernelSize, kValue, threshold, nonmaxKernelSize);
    //    printf("\n\nOutput of fast_corner_detector:\n");
    ...
    //        printf("\n\nInput:\n");
    //        displayPlanar(srcPtr, srcSize, channel);
    //        rppi_fast_corner_detector_u8_pln3_host(srcPtr, srcSize, dstPtr, gaussianKernelSize, stdDev, kernelSize, kValue, threshold, nonmaxKernelSize);
    //        printf("\n\nOutput of fast_corner_detector:\n");
    //        displayPlanar(dstPtr, srcSize, channel);
    //    }
    //    else if (type == 2)
    ...
    //        rppi_fast_corner_detector_u8_pkd3_host(srcPtr, srcSize, dstPtr, gaussianKernelSize, stdDev, kernelSize, kValue, threshold, nonmaxKernelSize);
    //        printf("\n\nOutput of fast_corner_detector:\n");
