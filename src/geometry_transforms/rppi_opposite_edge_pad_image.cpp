...
#include "cpu/host_opposite_edge_pad_image.hpp"
...
{
    opposite_edge_pad_image_host<Rpp8u>(static_cast<Rpp8u*>(srcPtr), srcSize, static_cast<Rpp8u*>(dstPtr), dstSize, padType, 
                            RPPI_CHN_PLANAR, 1);
...
int main(int argc, char** argv)
{
    ...
    do{
        printf("\nEnter padding type (1 = Top,Bottom, 2 = Left,Right): ");
        scanf("%d", &padType);
    }while (padType < 1 && padType > 2);
    ...
    if (input == 1)
    {
        ...
        if (padType == 1)
        {
            dstSize.height = srcSize.height + (2 * borderWidth);
            dstSize.width = srcSize.width;
        }
        else if (padType == 2)
        {
            dstSize.height = srcSize.height;
            dstSize.width = srcSize.width + (2 * borderWidth);
        }
        ...
        {   
            if (channel == 1)
            {
                ...
                rppi_opposite_edge_pad_image_u8_pln1_host(srcPtr, srcSize, dstPtr, dstSize, padType);
            ...
            else if (channel == 3)
            {
                ...
                start = high_resolution_clock::now();
                rppi_opposite_edge_pad_image_u8_pln3_host(srcPtrTemp, srcSize, dstPtrTemp, dstSize, padType);
                stop = high_resolution_clock::now();
        ...
        {   
            ...
            else if (channel ==3)
            {
                printf("\nExecuting pkd3...\n");
                start = high_resolution_clock::now();
                rppi_opposite_edge_pad_image_u8_pkd3_host(srcPtr, srcSize, dstPtr, dstSize, padType);
                stop = high_resolution_clock::now();
    ...
    if (matrix == 1)
    {
        ...
        rppi_opposite_edge_pad_image_u8_pln1_host(srcPtr, srcSize, dstPtr, dstSize, padType);
        printf("\n\nOutput of opposite_edge_pad_image:\n");
    ...
    else if (matrix == 2)
    {
        ...
        if (type == 1)
        {
            ...
            printf("\n\nInput:\n");
            displayPlanar(srcPtr, srcSize, channel);
            rppi_opposite_edge_pad_image_u8_pln3_host(srcPtr, srcSize, dstPtr, dstSize, padType);
            printf("\n\nOutput of opposite_edge_pad_image:\n");
            displayPlanar(dstPtr, dstSize, channel);
        }
        else if (type == 2)
