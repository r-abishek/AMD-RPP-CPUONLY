...
#include "cpu/host_random_crop_letterbox.hpp"
...
int main(int argc, char** argv)
{
    ...
    if (input == 1)
    {
        ...
        dstSize.width = (Rpp32u) RPPABS(xDiff) + (400);
        dstSize.height = (Rpp32u) RPPABS(yDiff) + (300);
        ...
        {   
            if (channel == 1)
            {
                ...
                rppi_random_crop_letterbox_u8_pln1_host(srcPtr, srcSize, dstPtr, dstSize, x1, y1, x2, y2);
            ...
            else if (channel == 3)
            {
                ...
                start = high_resolution_clock::now();
                rppi_random_crop_letterbox_u8_pln3_host(srcPtrTemp, srcSize, dstPtrTemp, dstSize, x1, y1, x2, y2);
                stop = high_resolution_clock::now();
        ...
        {   
            ...
            else if (channel ==3)
            {
                printf("\nExecuting pkd3...\n");
                start = high_resolution_clock::now();
                rppi_random_crop_letterbox_u8_pkd3_host(srcPtr, srcSize, dstPtr, dstSize, x1, y1, x2, y2);
                stop = high_resolution_clock::now();
    ...
    if (matrix == 1)
    {
        ...
        rppi_random_crop_letterbox_u8_pln1_host(srcPtr, srcSize, dstPtr, dstSize, x1, y1, x2, y2);
        printf("\n\nOutput of random_crop_letterbox:\n");
    ...
    else if (matrix == 2)
    {
        ...
        if (type == 1)
        {
            ...
            printf("\n\nInput:\n");
            displayPlanar(srcPtr, srcSize, channel);
            rppi_random_crop_letterbox_u8_pln3_host(srcPtr, srcSize, dstPtr, dstSize, x1, y1, x2, y2);
            printf("\n\nOutput of random_crop_letterbox:\n");
            displayPlanar(dstPtr, dstSize, channel);
        }
        else if (type == 2)
