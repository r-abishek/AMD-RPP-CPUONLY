...
#include "cpu/host_pad_image.hpp"
...
{
    pad_image_host<Rpp8u>(static_cast<Rpp8u*>(srcPtr), srcSize, static_cast<Rpp8u*>(dstPtr), dstSize,
                            RPPI_CHN_PLANAR, 1);
...
int main(int argc, char** argv)
{
    ...
    if (input == 1)
    {
        ...
        {   
            if (channel == 1)
            {
                ...
                rppi_pad_image_u8_pln1_host(srcPtr, srcSize, dstPtr, dstSize);
            ...
            else if (channel == 3)
            {
                ...
                start = high_resolution_clock::now();
                rppi_pad_image_u8_pln3_host(srcPtrTemp, srcSize, dstPtrTemp, dstSize);
                stop = high_resolution_clock::now();
        ...
        {   
            ...
            else if (channel ==3)
            {
                printf("\nExecuting pkd3...\n");
                start = high_resolution_clock::now();
                rppi_pad_image_u8_pkd3_host(srcPtr, srcSize, dstPtr, dstSize);
                stop = high_resolution_clock::now();
    ...
    else if (matrix == 2)
    {
        ...
        if (type == 1)
        {
            ...
            printf("\n\nInput:\n");
            displayPlanar(srcPtr, srcSize, channel);
            rppi_pad_image_u8_pln3_host(srcPtr, srcSize, dstPtr, dstSize);
            printf("\n\nOutput of pad_image:\n");
            displayPlanar(dstPtr, dstSize, channel);
        }
        else if (type == 2)
