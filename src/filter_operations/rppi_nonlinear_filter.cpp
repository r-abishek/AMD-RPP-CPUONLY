...
#include "cpu/host_nonlinear_filter.hpp"
...
RppStatus
rppi_nonlinear_filter_u8_pln1_host(RppPtr_t srcPtr, RppiSize srcSize, RppPtr_t dstPtr, Rpp32u kernelSize)
{
    nonlinear_filter_host<Rpp8u>(static_cast<Rpp8u*>(srcPtr), srcSize, static_cast<Rpp8u*>(dstPtr),
                     kernelSize,
                     RPPI_CHN_PLANAR, 1);
    return RPP_SUCCESS;
}
...
RppStatus
rppi_nonlinear_filter_u8_pln3_host(RppPtr_t srcPtr, RppiSize srcSize, RppPtr_t dstPtr, Rpp32u kernelSize)
{
    nonlinear_filter_host<Rpp8u>(static_cast<Rpp8u*>(srcPtr), srcSize, static_cast<Rpp8u*>(dstPtr),
                     kernelSize,
                     RPPI_CHN_PLANAR, 3);
...
RppStatus
rppi_nonlinear_filter_u8_pkd3_host(RppPtr_t srcPtr, RppiSize srcSize, RppPtr_t dstPtr, Rpp32u kernelSize)
{
    nonlinear_filter_host<Rpp8u>(static_cast<Rpp8u*>(srcPtr), srcSize, static_cast<Rpp8u*>(dstPtr),
                     kernelSize,
                     RPPI_CHN_PACKED, 3);
...
int main(int argc, char** argv)
{
    ...
    if (input == 1)
    {
        ...
        {   
            if (channel == 1)
            {
                ...
                rppi_nonlinear_filter_u8_pln1_host(srcPtr, srcSize, dstPtr, kernelSize);
            ...
            else if (channel == 3)
            {
                ...
                start = high_resolution_clock::now();
                rppi_nonlinear_filter_u8_pln3_host(srcPtrTemp, srcSize, dstPtrTemp, kernelSize);
                stop = high_resolution_clock::now();
        ...
        {   
            ...
            else if (channel ==3)
            {
                printf("\nExecuting pkd3...\n");
                start = high_resolution_clock::now();
                rppi_nonlinear_filter_u8_pkd3_host(srcPtr, srcSize, dstPtr, kernelSize);
                stop = high_resolution_clock::now();
    ...
    else if (matrix == 2)
    {
        ...
        if (type == 1)
        {
            ...
            printf("\n\nInput:\n");
            displayPlanar(srcPtr, srcSize, channel);
            rppi_nonlinear_filter_u8_pln3_host(srcPtr, srcSize, dstPtr, kernelSize);
            printf("\n\nOutput of nonlinear_filter:\n");
            displayPlanar(dstPtr, srcSize, channel);
        }
        else if (type == 2)
