...
#include "rppdefs.h"
#include "rppi_pixel_arrangement_conversions.h"
#include <stdio.h>
#include <stdlib.h>
#include <chrono>
#include "cpu/rpp_cpu_input_and_display.hpp"
#include "cpu/host_packed_to_planar.hpp"
#include "cpu/host_planar_to_packed.hpp"
...
int main(int argc, char** argv)
{
    ...
    if (input == 1)
    {
        ...
        rppi_packed_to_planar_u8_pkd3_host(srcPtr, srcSize, dstPtr);
        ...
        rppi_planar_to_packed_u8_pln3_host(srcPtrTemp, srcSize, dstPtr);
    ...
    if (matrix == 1)
    {
        ...
        auto start = high_resolution_clock::now();
        rppi_planar_to_packed_u8_pln1_host(srcPtr, srcSize, dstPtr);
        auto stop = high_resolution_clock::now();
        auto duration = duration_cast<nanoseconds>(stop - start);
        cout << "\nTime taken (nanoseconds) = " << duration.count() << endl;

        printf("\n\nOutput of planar_to_packed:\n");
    ...
    else if (matrix == 2)
    {
        ...
        auto start = high_resolution_clock::now();
        rppi_planar_to_packed_u8_pln3_host(srcPtr, srcSize, dstPtr);
        auto stop = high_resolution_clock::now();
        auto duration = duration_cast<nanoseconds>(stop - start);
        cout << "\nTime taken (nanoseconds) = " << duration.count() << endl;
